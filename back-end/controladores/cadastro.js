...
const { validarUsuario, validarCliente } = require("../validacao/usuarios");
...
const CadastrarCliente = async (req, res) => {
  ...
  try {
    const queryConsulta = "select * from clientes where email = $1";
    const { rowCount: quantidadeUsuario } = await conexao.query(queryConsulta, [
      email,
    ]);
    ...
    const ErroNaValidacaoDoUsuario = validarCliente(
      nome,
      email,
      senha
    );
...
module.exports = { CadastrarUsuario, CadastrarCliente };
