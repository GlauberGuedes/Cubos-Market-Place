...
export default function LoginUsuario() {
  ...
  const { setTokenUsuario, setUsuario, tokenUsuario } = useAuth();
  ...
  async function onSubmit(data) {
    ...
    setOpenLoading(true);
    try {
      const resposta = await fetch("http://localhost:8000/login-usuario", {
        method: "POST",
    ...
    <div className={classes.login}>
      <form className={classes.form} onSubmit={handleSubmit(onSubmit)}>
        ...
        <Typography variant="body2" component="p">
          Primeira vez aqui? <NavLink to="/cadastro-usuario">CRIE UMA CONTA</NavLink>
        </Typography>
      </form>
